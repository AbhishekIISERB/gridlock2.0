"""
00_preprocess.py — Raw data preprocessing and DBSCAN spatial clustering

Reads : jan_to_may_police_violation_anonymized.csv  (raw dataset provided)
Writes: parking_clustered.csv  — full dataset with cluster labels + feature engineering
        cluster_stats.csv      — per-cluster aggregate statistics
...
import ast, re, warnings, os
warnings.filterwarnings('ignore')

# Resolve all paths relative to the project root (one level up from pipeline/)
...
raw = pd.read_csv(os.path.join(ROOT, "jan_to_may_police_violation_anonymized.csv"),
                  parse_dates=['created_datetime','modified_datetime','validation_timestamp'],
                  low_memory=False)
...
raw['week']     = raw['created_datetime'].dt.isocalendar().week.astype(int)
...
agg.to_csv(os.path.join(ROOT, "cluster_stats.csv"), index=False)
top_zones.to_csv(os.path.join(ROOT, "top_enforcement_zones.csv"), index=False)
raw.to_csv(os.path.join(ROOT, "parking_clustered.csv"), index=False)
print("\nSaved all outputs to project root.")
print("  cluster_stats.csv")
print("  top_enforcement_zones.csv")
print("  parking_clustered.csv")
